#include "EllipseFunction.h"

EllipseFunction::EllipseFunction(double aConstr, double bConstr, int numberPointsConstr, double offSetXConstr,
 ...
 */
void EllipseFunction::UpdateNumberPoints(int newNumberPoints) {
 ...
 */
double *EllipseFunction::GetXArray() {
 ...
 */
double *EllipseFunction::GetYArray() {
    //returned an int3 exception here??? Literally no idea why, maybe define these arrays before, or add an error handler???
    double *returnArray = new double[numberPoints];
    for (int i = 0; i < numberPoints; i++) {
 ...
 */
int EllipseFunction::GetNumberPoints() {
    return numberPoints;
}
